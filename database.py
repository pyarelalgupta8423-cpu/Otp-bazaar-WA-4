...
def get_support_url():
    ...
    if not url.startswith(("http://", "https://")):
        return "https://t.me/tgtelehelpbot"
    return url


def get_country_description(country_name: str) -> str:
    res = settings_col.find_one({"key": f"country_desc:{country_name}"})
    if res and res.get("value"):
        return str(res["value"])
    return ""


def set_country_description(country_name: str, text: str):
    settings_col.update_one(
        {"key": f"country_desc:{country_name}"},
        {"$set": {"value": text, "country": country_name}},
        upsert=True,
    )
