...
from database import users_col, stock_col, deposits_col, is_bot_online, is_admin, has_perm, ADMIN_ID, get_flag_by_country_name, get_country_info, update_balance
from config import logger, PE_CROWN, PE_LOCATION, PE_LIGHTNING, P_USERS, P_PKG, P_WAIT, P_ON, P_YES, P_NO, P_WARN, P_DOC, P_FLAG, P_MONEY, P_PHONE, P_GLOBE, P_2FA, P_CAL, P_OTP, P_CARD, P_TG, P_ACC, P_USDT, P_UPI, P_CART, P_GIFT, P_STATS, P_OFF, API_ID, API_HASH, bot
...
async def admin_panel_handler(event):
    ...
    total_stock = stock_col.count_documents({"available": 1})
    pending_deposits = deposits_col.count_documents({"status": "pending"})
    btns = []
    ...
    if uid == ADMIN_ID or has_perm(uid, 'p_manage_stock'):
        ...
        r2.append(style_btn("Export All ZIP", "adm_exportall", "primary", icon=5355292788923593967))
        r2.append(style_btn("WhatsApp Services", "adm_waservices", "success", icon=5408995930416362034))
    ...
    if uid == ADMIN_ID or has_perm(uid, 'p_settings'):
        ...
        btns.append(r5)
        btns.append([style_btn("Country Desc", "adm_countrydesc", "primary", icon=5408995930416362034), style_btn("Support URL", "adm_supporturl", "primary", icon=5409098988156629257), style_btn("Payments", "adm_payments", "primary", icon=5409098988156629257)])
    ...
    try:
        await event.edit(header, buttons=btns)
    except Exception as _e:
        logger.exception("admin panel edit failed: %s", _e)
