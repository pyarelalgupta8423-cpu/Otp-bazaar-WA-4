    ...
    async with bot.conversation(chat, timeout=600) as conv:
        ...
        try:
            if action_data == "ap_add_country" and has_perm(uid, 'p_manage_stock'):
                ...
                qr_msg = await get_reply(f"📸 <b>Send QR Code Image:</b>\n<i>(Or type <code>skip</code> if no QR needed)</i>", allow_file=True)
                qr_path = ""
                if qr_msg.photo:
                ...
                resp = await get_reply(f"{P_PKG} <b>Send the ZIP file containing <code>.session</code> files:</b>", allow_file=True)
