...
from database import users_col, stock_col, orders_col, get_flag_by_country_name, get_country_description, db as mongo_db
from config import (
...
async def show_years(event, mode, country):
    years_pipe = [
        ...
        {"$group": {
            "_id": {
                "year": "$account_year",
                "price": "$price",
                "type": {"$ifNull": ["$account_type", "Fresh"]},
            },
            "count": {"$sum": 1},
        }},
        {"$sort": {"_id.year": 1, "_id.type": 1, "_id.price": 1}},
    ]
    years = [
        (r["_id"]["year"], r["count"], r["_id"]["price"], r["_id"].get("type") or "Fresh")
    ...
    desc = get_country_description(country)
    btns = [[
        style_btn("Type", "noop", "primary", icon=5408995930416362034),
        style_btn("Price", "noop", "primary", icon=5409098988156629257),
        style_btn("Stock", "noop", "primary", icon=6129627894349045589),
    ]]
    for y, count, price, acc_type in years:
        cb = f"by|{mode}|{country}|{y}|{price}|{acc_type}"
        label = f"{acc_type} · {y}"
        btns.append([
            style_btn(label[:24], cb, "primary", icon=5408995930416362034),
    ...
    msg = f"<blockquote>{flag} <b>{country}</b> — stock (ascending)</blockquote>"
    if desc:
        msg += "\n<blockquote>" + str(desc) + "</blockquote>"
    msg += "\n<blockquote>Tap Type / Price / Stock — same option.</blockquote>"
...
async def confirm_purchase(event, country, year, price, acc_type="Fresh"):
    msg = (
    ...
    btns = [
        [style_btn(
            "𝐂ᴏɴғɪʀᴍ 𝐁ᴜʏ",
            f"buy_cf|{country}|{year}|{price}|{acc_type}",
            "success",
...
async def process_purchase(event, country, year, price_str, acc_type="Fresh"):
...
async def show_bulk_qty(event, country, year, price, available, acc_type="Fresh"):
    ...
    for q in presets:
        if q <= max_qty:
            row.append(
                style_btn(
                    f"x{q}",
                    f"bulk_qty|{country}|{year}|{price}|{acc_type}|{q}",
                    "primary",
...
async def confirm_bulk(event, country, year, price, qty, acc_type="Fresh"):
    flag = get_flag_by_country_name(country)
    ...
    btns = [
        [
            style_btn(
                ...
                f"bulk_cf|{country}|{year}|{price}|{acc_type}|{qty}",
                "success",
                icon=5409320020058584473,
            )
        ],
...
async def process_bulk_purchase(event, country, year, price_str, qty_str, acc_type="Fresh"):
    """
    ...
    async with get_user_lock(uid):
        ...
        for _ in range(qty):
            ...
            if (acc_type or "Fresh") != "Fresh":
                q["account_type"] = acc_type
            else:
                q["$or"] = [
                    {"account_type": "Fresh"},
                    {"account_type": {"$exists": False}},
...
def register_buy(bot):
    ...
    async def cb_by(e):
        ...
        mode, country, year, price = g(1), g(2), g(3), g(4)
        acc_type = g(5) or "Fresh"
        ...
        if mode == "bulk":
            ...
            avail = stock_col.count_documents(q)
            await show_bulk_qty(e, country, year, price, avail, acc_type)
        else:
            await confirm_purchase(e, country, year, price, acc_type)
    ...
    async def cb_bulk_qty(e):
        ...
        def g(i):
            v = p.group(i)
            return v.decode() if isinstance(v, (bytes, bytearray)) else v
        country, year, price, acc_type, qty = g(1), g(2), g(3), g(4), g(5)
        await confirm_bulk(e, country, year, price, qty, acc_type)
    ...
    async def cb_buy_cf(e):
        ...
        def g(i):
            ...
            if v is None:
                return None
            return v.decode() if isinstance(v, (bytes, bytearray)) else v
        country, year, price = g(1), g(2), g(3)
        acc_type = g(4) or "Fresh"
        await process_purchase(e, country, year, price, acc_type)
    ...
    async def cb_bulk_cf(e):
        ...
        def g(i):
            ...
            return v.decode() if isinstance(v, (bytes, bytearray)) else v
        country, year, price, acc_type, qty = g(1), g(2), g(3), g(4), g(5)
        await process_bulk_purchase(e, country, year, price, qty, acc_type)
